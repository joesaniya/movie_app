...
class _UserListScreenState extends State<UserListScreen>
  ...
  void _onScroll() {
    ...
    if (pos.pixels >= (pos.maxScrollExtent * 0.8) - 500 &&
        p.hasMore &&
        !p.isLoading) {
      p.fetchUsers();
    }
                ...
                Consumer<ConnectivityProvider>(
                  builder: (ctx, conn, _) => Column(
                    children: [
                      ...
                      if (_showConnectingIndicator && conn.isOnline)
                        const lw.ConnectingIndicator(),
                    ],
                  ),
                ),
                    ...
                    builder: (ctx, provider, _) {
                      ...
                      if (provider.allUsers.isEmpty) {
                        return lw.EmptyWidget(
                          ...
                          message:
                              'Create a profile to start discovering films.',
                                ...
                                Navigator.push(
                                  ...
                                  AnimatedPageRoute(
                                    builder: (_) => MultiProvider(
                                      providers: [
                                        ChangeNotifierProvider.value(value: bk),
                                      ],
        ...
        children: [
          Row(
            children: [
              ...
              Consumer<ConnectivityProvider>(
                builder: (_, conn, __) =>
                    ConnectivityDot(isOnline: conn.isOnline),
              ),
            ],
          ),
          ...
          const Text(
            'Select your profile to browse the collection.',
            style: TextStyle(
              fontFamily: 'DMSans',
              fontSize: 13,
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: () {
            ...
            Navigator.push(
              context,
              AnimatedPageRoute(builder: (_) => const AddUserScreen()),
            ).then((_) {
              if (mounted) p.loadLocalUsers();
            });
          },
          borderRadius: BorderRadius.circular(AppTheme.radiusLg),
          child: const Padding(
                    ...
                    Row(
                      children: [
                        ...
                        if (widget.user.id != null)
                          Text(
                            'ID #${widget.user.id.toString().padLeft(3, '0')}',
                            style: const TextStyle(
                              fontFamily: 'DMSans',
                              fontSize: 10,
                              color: AppTheme.dustGray,
